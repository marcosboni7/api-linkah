...
exports.criarSessaoCheckout = async (req, res) => {
    try {
        ...
        // 4. LÓGICA DE SPLIT (Sua comissão de 5%)
...
exports.vincularContaStripe = async (req, res) => {
    try {
        ...
        // 1. Criar a conta Express no Stripe
        ...
        // 2. Salvar o account_id no perfil do usuário
        await db.query(
            "UPDATE public.usuarios SET stripe_account_id = $1 WHERE email = $2",
            [account.id, email]
        );

        // 3. Gerar link de onboarding para o produtor preencher dados bancários
        const accountLink = await stripe.accountLinks.create({
            account: account.id,
            refresh_url: `${process.env.FRONTEND_URL}/dashboard/configuracoes`,
            return_url: `${process.env.FRONTEND_URL}/dashboard/configuracoes?stripe=success`,
            type: 'account_onboarding',
        });

        res.json({ url: accountLink.url });
    } catch (err) {
        console.error("❌ Erro no Onboarding Stripe:", err.message);
...
// --- 4. BUSCAR DETALHES DA COMPRA (SUCESSO) ---
...
// --- 5. LISTAR MEUS INGRESSOS (USUÁRIO) ---
exports.listarMeusIngressos = async (req, res) => {
    try {
        ...
        const result = await db.query(
            `SELECT id, evento_nome as evento, TO_CHAR(data_evento, 'DD/MM/YYYY') as data, 
                    status, quantidade as qtd, stripe_session_id
             FROM public.compras 
